...
class DatabaseService {
  ...
  Future ? updateworkerdata(String ?Username, String ?Phoneno, String ?email,List<String>works,String url) async {
  ...
  Future ? updateuserdata(String ?Username, String ?Phoneno, String ?email,String ? url) async {
    return await userdetails.doc(uid).set(
        {
          'uid':uid,
          'photoURL':url,
  ...
  Future ? FindWorkers(String Key)async{
    var data  = await userdetails.where('type',isEqualTo: 1).where('works',arrayContains: Key).get();

   return data.docs;

  }
  Future ?setURL(String URL)async{
   return await userdetails.doc(uid).update(
      {
        'photoURL':URL
      }
    );
